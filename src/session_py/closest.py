...
def _surface_newton(
    ...
    for _ in range(max_iterations):
        ...
        dv_vec = derivs[1]
        delta = Vector(
            test_point[0] - pt[0], test_point[1] - pt[1], test_point[2] - pt[2]
        )
        fu = -delta.dot(du_vec)
        fv = -delta.dot(dv_vec)
...
def _closest_point_on_triangle(p: Point, a: Point, b: Point, c: Point) -> Point:
    ...
    if va <= 0.0 and (d4 - d3) >= 0.0 and (d5 - d6) >= 0.0:
        w = (d4 - d3) / ((d4 - d3) + (d5 - d6))
        return Point(
            b[0] + w * (c[0] - b[0]), b[1] + w * (c[1] - b[1]), b[2] + w * (c[2] - b[2])
        )
    denom = 1.0 / (va + vb + vc)
    v = vb * denom
    w = vc * denom
    return Point(
        a[0] + abx * v + acx * w, a[1] + aby * v + acy * w, a[2] + abz * v + acz * w
    )
...
class Closest:
    ...
    def mesh_point_aabb(mesh: Mesh, test_point: Point) -> tuple[Point, int, float]:
        """Closest point, face key and distance on a mesh via a triangle AABB tree"""
        best_point = Point(0, 0, 0)
        best_face_key = 0
        best_dist = math.inf
        if mesh.number_of_faces() == 0:
