...
class SpatialKDTree:
    ...
    def _nth_element(self, indices: list[int], lo: int, mid: int, hi: int, axis: int) -> None:
        while hi - lo > 1:
    ...
    def _insert_sorted(self, best: list[tuple[int, float]], idx: int, d2: float, k: int) -> None:
        pos = len(best)
    ...
    def radius_search(self, query: Point, radius: float) -> list[tuple[int, float]]:
        ...
        while len(stack) > 0:
            ...
            self._push(stack, far, diff * diff)
            self._push(stack, near, 0.0)
        result.sort(key=lambda hit: hit[1])
        return result
