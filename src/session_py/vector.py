...
class Vector:
    ...
    def __setitem__(self, index: int, value: float) -> None:
        """Set coordinate by index; drops the cached magnitude"""
        if index == 0:
            self._x = value
        elif index == 1:
            self._y = value
    ...
    def average_normal(points: list["Point"]) -> "Vector":
        ...
        n = len(points) - 1 if dx * dx + dy * dy + dz * dz < 1e-10 else len(points)
        normal = Vector(0.0, 0.0, 0.0)
        for i in range(n):
            prev = (i + n - 1) % n
            next = (i + 1) % n
            ax = points[i][0] - points[prev][0]
            ay = points[i][1] - points[prev][1]
            az = points[i][2] - points[prev][2]
            bx = points[next][0] - points[i][0]
            by = points[next][1] - points[i][1]
            bz = points[next][2] - points[i][2]
