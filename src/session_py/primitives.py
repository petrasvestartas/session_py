...
class Primitives:
    ...
    @staticmethod
    def _cubic_interpolation_curve(points, params):
        n = len(points)
        if n < 2:
            return NurbsCurve()
        dim = 3
        ...
        if degree < 3:
            cv_count = n
            if degree == 1:
                result = NurbsCurve(dim, False, order, cv_count)
                for i in range(n):
                    result.set_knot(i, params[i])
                for i in range(n):
                    result.set_cv(i, points[i])
                return result
            # degree 2: averaged interior knots
            kn_count = cv_count + order - 2
            kn = [0.0] * kn_count
            for i in range(order - 1):
                kn[i] = params[0]
            for i in range(kn_count - order + 1, kn_count):
                kn[i] = params[n - 1]
            for j in range(1, n - order + 1):
                s = sum(params[j:j + degree]) / degree
                kn[order - 2 + j] = s
            A = [[0.0] * n for _ in range(n)]
            for k in range(n):
                span = knot.find_span(order, cv_count, kn, params[k])
                basis = knot.eval_basis(order, kn, span, params[k])
                for jj in range(order):
                    if span + jj < n:
                        A[k][span + jj] = basis[jj]
            cv = [0.0] * (n * dim)
            for i in range(n):
                for d in range(dim):
            ...
            for i in range(n - 1, -1, -1):
                for d in range(dim):
                    ...
                    for jj in range(i + 1, n):
                        s -= A[i][jj] * cv[jj*dim+d]
                    cv[i*dim+d] = s / A[i][i]
            result = NurbsCurve(dim, False, order, cv_count)
            for i in range(kn_count):
                result.set_knot(i, kn[i])
            for i in range(cv_count):
        ...
        cv_count = n + 2
        knots_arr = knot.build_interp_knots(np.array(params), degree)
        kc = len(knots_arr)
        knots_list = list(knots_arr)
        ...
        def estimate_tangent(i0, i1, i2):
            ...
            if denom < 1e-16:
                ...
                return Vector(dx/ln, dy/ln, dz/ln) if ln > 0 else Vector(0,0,0)
        ...
        if n >= 3:
            ...
            ln = math.sqrt(dx*dx+dy*dy+dz*dz)
            if ln > 0:
                tan_start = Vector(dx/ln, dy/ln, dz/ln)
                tan_end = tan_start
            else:
        ...
        for i in range(1, n - 1):
            basis = knot.eval_basis(order, knots_list, i, params[i])
            lower[i] = basis[0]
            diag[i] = basis[1]
            upper[i] = basis[2]
            for d in range(dim):
        ...
        solution = knot.solve_tridiagonal(dim, sys_n, lower, diag, upper, rhs)
        ...
        result = NurbsCurve(dim, False, order, cv_count)
        for i in range(kc):
            result.set_knot(i, knots_list[i])
        for i in range(cv_count):
            result.set_cv(i, Point(cv[i*3], cv[i*3+1], cv[i*3+2]))
        return result

    @staticmethod
    def _create_loft_with_params(input_curves, cross_params):
        curves = [c.duplicate() for c in input_curves]
        Primitives._make_curves_compatible(curves)

        n_sections = len(curves)
        cv_u = curves[0].cv_count()
        is_rat = curves[0].is_rational()

        col_curves = []
        for col in range(cv_u):
            col_pts = []
            for k in range(n_sections):
                if is_rat:
                    x, y, z, w = curves[k].get_cv_4d(col)
                    col_pts.append(Point(x/w, y/w, z/w))
                else:
                    col_pts.append(curves[k].get_cv(col))
            col_curves.append(Primitives._cubic_interpolation_curve(col_pts, cross_params))

        cv_v = col_curves[0].cv_count()
        order_v = col_curves[0].order()

        srf = NurbsSurface.create_raw(3, is_rat, curves[0].order(), order_v, cv_u, cv_v)
        for i in range(srf.knot_count(0)):
            srf.set_knot(0, i, curves[0].knot(i))
        for i in range(srf.knot_count(1)):
    ...
    @staticmethod
    def _create_bicubic_grid_surface(grid, u_params, v_params):
        n_rows = len(grid)
        row_curves = []
        for i in range(n_rows):
            row_curves.append(Primitives._cubic_interpolation_curve(grid[i], u_params))
        Primitives._make_curves_compatible(row_curves)

        cv_u = row_curves[0].cv_count()
        order_u = row_curves[0].order()

        col_curves = []
        for col in range(cv_u):
            col_pts = []
            for row in range(n_rows):
                col_pts.append(row_curves[row].get_cv(col))
            col_curves.append(Primitives._cubic_interpolation_curve(col_pts, v_params))

        cv_v = col_curves[0].cv_count()
        order_v = col_curves[0].order()

        srf = NurbsSurface.create_raw(3, False, order_u, order_v, cv_u, cv_v)
        for i in range(srf.knot_count(0)):
    ...
    @staticmethod
    def _make_surfaces_knot_compatible_1d(a, b, c):
        d = 0
        max_deg = max(a.degree(d), b.degree(d), c.degree(d))
        if a.degree(d) < max_deg: a.increase_degree(d, max_deg)
        if b.degree(d) < max_deg: b.increase_degree(d, max_deg)
        if c.degree(d) < max_deg: c.increase_degree(d, max_deg)

        a.set_domain(d, 0.0, 1.0)
        b.set_domain(d, 0.0, 1.0)
        c.set_domain(d, 0.0, 1.0)

        ka = list(a.get_knots(d))
        kb = list(b.get_knots(d))
        kc = list(c.get_knots(d))
        unified = Primitives._merge_knot_vectors(
            Primitives._merge_knot_vectors(ka, kb), kc)

        def insert_missing(srf, dr, uni):
            orig = list(srf.get_knots(dr))
            ci = 0
            for ui in range(len(uni)):
                if ci < len(orig) and abs(orig[ci] - uni[ui]) < 1e-10:
                    ci += 1
                else:
                    srf.insert_knot(dr, uni[ui], 1)

        insert_missing(a, d, unified)
        insert_missing(b, d, unified)
        insert_missing(c, d, unified)

    @staticmethod
    def _find_curve_intersection(crv1, crv2):
        a1, b1 = crv1.domain()
        a2, b2 = crv2.domain()
        n = 80
        best_d2, t1, t2 = 1e30, a1, a2
        ...
        for i in range(n + 1):
            ...
            for j in range(n + 1):
                ...
                d2 = dx*dx + dy*dy + dz*dz
                if d2 < best_d2:
                    best_d2, t1, t2 = d2, s1, s2

        dt1, dt2 = (b1 - a1) / n, (b2 - a2) / n
        for _ in range(4):
            lo1, hi1 = max(a1, t1 - dt1), min(b1, t1 + dt1)
            lo2, hi2 = max(a2, t2 - dt2), min(b2, t2 + dt2)
            for i in range(21):
                s1 = lo1 + (hi1 - lo1) * i / 20
                p1 = crv1.point_at(s1)
                for j in range(21):
        ...
        p1, p2 = crv1.point_at(t1), crv2.point_at(t2)
        pt = Point((p1[0]+p2[0])/2, (p1[1]+p2[1])/2, (p1[2]+p2[2])/2)
        return t1, t2, pt

    @staticmethod
    def _make_surfaces_knot_compatible(a, b, c):
        for d in range(2):
            max_deg = max(a.degree(d), b.degree(d), c.degree(d))
            if a.degree(d) < max_deg: a.increase_degree(d, max_deg)
            if b.degree(d) < max_deg: b.increase_degree(d, max_deg)
            ...
            def merge_kv(x, y):
                ...
                while i < len(x) and j < len(y):
                    if abs(x[i] - y[j]) < 1e-10:
                        merged.append(x[i]); i += 1; j += 1
                    elif x[i] < y[j]:
                        merged.append(x[i]); i += 1
                    else:
                        merged.append(y[j]); j += 1
                while i < len(x): merged.append(x[i]); i += 1
                while j < len(y): merged.append(y[j]); j += 1
                return merged

            unified = merge_kv(merge_kv(ka, kb), kc)

            def insert_missing(srf, dir_, uni):
                orig = list(srf.get_knots(dir_))
                ci = 0
                for ui in range(len(uni)):
                    if ci < len(orig) and abs(orig[ci] - uni[ui]) < 1e-10:
                        ci += 1
                    else:
                        srf.insert_knot(dir_, uni[ui], 1)

            insert_missing(a, d, unified)
            insert_missing(b, d, unified)
            insert_missing(c, d, unified)

        any_rat = a.is_rational() or b.is_rational() or c.is_rational()
        if any_rat:
            a.make_rational()
            b.make_rational()
            c.make_rational()
    ...
    def create_network(u_curves, v_curves):
        ...
        for c in u_curves:
            if not c.is_valid(): return NurbsSurface()
        for c in v_curves:
            if not c.is_valid(): return NurbsSurface()

        # Step 1: Find intersection parameters
        u_par = [[0.0]*n_v for _ in range(n_u)]
        v_par = [[0.0]*n_v for _ in range(n_u)]
        for i in range(n_u):
            for j in range(n_v):
                t1, t2, _ = Primitives._find_curve_intersection(u_curves[i], v_curves[j])
                u_par[i][j] = t1
                v_par[i][j] = t2

        # Step 2: Build point grid with averaging
        grid = [[None]*n_v for _ in range(n_u)]
        for i in range(n_u):
            for j in range(n_v):
                pu = u_curves[i].point_at(u_par[i][j])
                pv = v_curves[j].point_at(v_par[i][j])
                grid[i][j] = Point(0.5*(pu[0]+pv[0]), 0.5*(pu[1]+pv[1]), 0.5*(pu[2]+pv[2]))
        ...
        v_arc_params = [0.0] * n_u
        for i in range(1, n_u):
            mx, mn = 0.0, 1e30
            for j in range(n_v):
                d = pdist(grid[i-1][j], grid[i][j])
                mx = max(mx, d)
                mn = min(mn, d)
            inc = (math.sqrt(mx) + math.sqrt(mn)) * 0.5
            inc = inc * inc
            if inc < 0.0001: inc = 0.0001
            v_arc_params[i] = v_arc_params[i-1] + inc
        if v_arc_params[-1] > 1e-14:
            for i in range(n_u): v_arc_params[i] /= v_arc_params[-1]

        u_arc_params = [0.0] * n_v
        for j in range(1, n_v):
            mx, mn = 0.0, 1e30
            for i in range(n_u):
                d = pdist(grid[i][j-1], grid[i][j])
                mx = max(mx, d)
                mn = min(mn, d)
            inc = (math.sqrt(mx) + math.sqrt(mn)) * 0.5
            inc = inc * inc
        ...
        if u_arc_params[-1] > 1e-14:
            for j in range(n_v): u_arc_params[j] /= u_arc_params[-1]

        # Step 4: Build grid surface T
        T = Primitives._create_bicubic_grid_surface(grid, u_arc_params, v_arc_params)

        # Step 5: Build L_u (loft through u-curves)
        u_crvs = [c.duplicate() for c in u_curves]
        L_u = Primitives._create_loft_with_params(u_crvs, v_arc_params)

        # Step 6: Build L_v with reparametrized v-curves, then transpose
        def reparametrize_curve(curve, old_params, new_params, samples_per_span=10):
            n_pts = len(old_params)
            pts = []
            pars = []
            for ii in range(n_pts - 1):
                t0, t1 = old_params[ii], old_params[ii + 1]
                a0, a1 = new_params[ii], new_params[ii + 1]
                nsamp = samples_per_span + 1 if ii == n_pts - 2 else samples_per_span
                for s in range(nsamp):
                    frac = s / samples_per_span
                    pts.append(curve.point_at(t0 + (t1 - t0) * frac))
                    pars.append(a0 + (a1 - a0) * frac)
            return Primitives._cubic_interpolation_curve(pts, pars)

        v_repar = []
        for j in range(n_v):
            old_p = [v_par[i][j] for i in range(n_u)]
            new_p = list(v_arc_params)
            v_repar.append(reparametrize_curve(v_curves[j], old_p, new_p))
        L_v = Primitives._create_loft_with_params(v_repar, u_arc_params)
        L_v.transpose()

        if not T.is_valid() or not L_u.is_valid() or not L_v.is_valid():
            return NurbsSurface()

        # Step 7: Knot compatibility via transpose trick
        Primitives._make_surfaces_knot_compatible_1d(T, L_u, L_v)
        T.transpose(); L_u.transpose(); L_v.transpose()
        Primitives._make_surfaces_knot_compatible_1d(T, L_u, L_v)
        T.transpose(); L_u.transpose(); L_v.transpose()

        any_rat = T.is_rational() or L_u.is_rational() or L_v.is_rational()
        if any_rat:
            T.make_rational()
            L_u.make_rational()
            L_v.make_rational()
        ...
        surface = T.duplicate()
        for i in range(surface.cv_count_dir(0)):
            for j in range(surface.cv_count_dir(1)):
                if any_rat:
                    lux, luy, luz, luw = L_u.get_cv_4d(i, j)
                    lvx, lvy, lvz, lvw = L_v.get_cv_4d(i, j)
                    tx, ty, tz, tw = T.get_cv_4d(i, j)
