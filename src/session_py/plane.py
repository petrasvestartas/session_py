...
class Plane:
    ...
    def is_same_direction(
        ...
        """Normals parallel (can_be_flipped) or exactly opposite (not can_be_flipped)"""
    ...
    def to_polylines(self, scale: float = 1.0) -> list["Polyline"]:
        ...
        x_line = Polyline(
            [origin_pt, Point(o[0] + x[0] * s, o[1] + x[1] * s, o[2] + x[2] * s)]
        )
        x_line.linecolor = Color.red()
        y_line = Polyline(
            [origin_pt, Point(o[0] + y[0] * s, o[1] + y[1] * s, o[2] + y[2] * s)]
        )
        y_line.linecolor = Color.green()
        z_line = Polyline(
            [origin_pt, Point(o[0] + z[0] * s, o[1] + z[1] * s, o[2] + z[2] * s)]
        )
