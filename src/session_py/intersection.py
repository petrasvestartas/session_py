...
def plane_plane_plane(
    plane0: "Plane", plane1: "Plane", plane2: "Plane"
) -> Point | None:
    """
...
def _curve_signed_distance_to_plane(pt, plane):
    ...
    v = Vector(
        pt[0] - plane.origin[0], pt[1] - plane.origin[1], pt[2] - plane.origin[2]
    )
...
def plane_plane_plane_check(
    p0: "Plane", p1: "Plane", p2: "Plane", angle_tol: float = 0.1
) -> Point | None:
...
def plane_4lines(
    plane: "Plane", l0: Line, l1: Line, l2: Line, l3: Line
) -> object | None:
...
def get_quad_from_line_topbottomplanes(
...
def get_orthogonal_vector_between_two_plane_pairs(
    ...
    result = line_line_parameters(
        l0, l1, 0.0, intersect_segments=False, near_parallel_as_closest=True
    )
    if result is None:
        return None
    t0, t1 = result
...
def polyline_plane_to_line(
    poly: "Polyline", plane: "Plane", align_start: bool
) -> object | None:
    ...
    d0 = (
        (p0[0] - align_start[0]) ** 2
        + (p0[1] - align_start[1]) ** 2
        + (p0[2] - align_start[2]) ** 2
    )
    d1 = (
        (p1[0] - align_start[0]) ** 2
        + (p1[1] - align_start[1]) ** 2
        + (p1[2] - align_start[2]) ** 2
    )
    if d1 < d0:
        p0, p1 = p1, p0
...
def closed_and_open_paths_2d(
    ...
    xax = plane.x_axis
    yax = plane.y_axis
    xax = Vector(xax[0], xax[1], xax[2])
    xax.normalize_self()
    yax = Vector(yax[0], yax[1], yax[2])
    yax.normalize_self()
    ...
    if plate_n > 1:
        ...
        if (
            abs(f[0] - l[0]) < 1e-6
            and abs(f[1] - l[1]) < 1e-6
            and abs(f[2] - l[2]) < 1e-6
        ):
            plate_n -= 1
    ...
    for piece in pieces:
        ...
        if count == 0:
            ...
            pts = list(piece)
            if (
                sq2(c2d[-1], pts[0]) > DISTANCE_SQ
                and sq2(c2d[-1], pts[-1]) > DISTANCE_SQ
            ):
                c2d.reverse()
