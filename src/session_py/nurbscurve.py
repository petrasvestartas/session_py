...
class NurbsCurve:
    ...
    @staticmethod
    def create_interpolated(points: List[Point], degree: int = 3,
                           closed: bool = False, knot_style: int = 1) -> 'NurbsCurve':
        ...
        closed : bool, optional
            If True, creates a closed curve. Defaults to False.
        knot_style : int, optional
            Parameterization style: 0=uniform, 1=chord-length (default), 2=centripetal.
        ...
        if degree != 3 or closed:
            return NurbsCurve.create(closed, degree, points)
        ...
        if min_dist <= max_dist * 1.49e-08:
            return NurbsCurve.create(False, degree, points)

        # Step 2: Build clamped knot vector
        knots = np.zeros(n + order)
        ...
        for row in range(n):
            t = params[row]
            span = NurbsCurve._find_interp_span(t, n, degree, knots)
            N = NurbsCurve._evaluate_cubic_basis(t, span, knots)

            for j in range(degree + 1):
                col = span - degree + j
                if 0 <= col < n:
                    N_matrix[row][col] = N[j]

            rhs_x[row] = points[row].x
            rhs_y[row] = points[row].y
            rhs_z[row] = points[row].z
        ...
        cv_x = NurbsCurve._solve_tridiagonal(n, lower.copy(), diag.copy(), upper.copy(), rhs_x)
        cv_y = NurbsCurve._solve_tridiagonal(n, lower.copy(), diag.copy(), upper.copy(), rhs_y)
        cv_z = NurbsCurve._solve_tridiagonal(n, lower.copy(), diag.copy(), upper.copy(), rhs_z)

        if cv_x is None or cv_y is None or cv_z is None:
            return NurbsCurve.create(False, degree, points)

        # Step 4: Create curve
        result = NurbsCurve()
        result.create_curve(3, False, order, n)

        for i in range(n):
            result.set_cv(i, Point(cv_x[i], cv_y[i], cv_z[i]))

        # Set knots (stored knots = cv_count + order - 2)
        for i in range(n + order - 2):
            result.set_knot(i, knots[i + 1])

        return result

    @staticmethod
    def _solve_tridiagonal(n: int, lower: np.ndarray, diag: np.ndarray,
                          upper: np.ndarray, rhs: np.ndarray) -> Optional[np.ndarray]:
    ...
    @staticmethod
    def _find_interp_span(t: float, cv_count: int, degree: int, knots: np.ndarray) -> int:
        """Find knot span for interpolation."""
        n = cv_count - 1
        if t >= knots[n + 1]:
            return n
        if t <= knots[degree]:
    ...
    @staticmethod
    def _evaluate_cubic_basis(t: float, span: int, knots: np.ndarray) -> np.ndarray:
        """Evaluate cubic B-spline basis at parameter t."""
        N = np.zeros(4)
        N[0] = 1.0
