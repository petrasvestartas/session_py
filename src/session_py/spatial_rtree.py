...
class SpatialRTree:
    ...
    def search(self, a_min: list[float], a_max: list[float], a_callback: Callable[[int], bool]) -> int:
        rect = self._make_rect(a_min, a_max)
        count = [0]
        self._search_internal(rect, self._m_root, count, a_callback)
        return count[0]
    ...
    def _insert_rect_rec(self, branch: _Branch, node: _Node, level: int) -> _Node | None:
        if node.m_level == level:
            return self._add_branch(branch, node)
        ...
        new_b.m_child = other
        return self._add_branch(new_b, node)
    ...
    def _search_internal(self, rect: _Rect, node: _Node, count: list[int], callback: Callable[[int], bool]) -> bool:
        for i in range(node.m_count):
            if not self._overlaps(rect, node.m_branch[i].m_rect):
                continue
            if node.is_leaf():
                count[0] += 1
                if not callback(node.m_branch[i].m_data):
                    return False
            elif not self._search_internal(rect, node.m_branch[i].m_child, count, callback):
                return False
        return True

    def _remove_rect_internal(self, rect: _Rect, data: int, node: _Node, reinsert_list: list[_Node]) -> bool:
        for i in range(node.m_count):
            if not self._overlaps(rect, node.m_branch[i].m_rect):
                continue
            if node.is_leaf():
                ...
                return True
            if not self._remove_rect_internal(rect, data, node.m_branch[i].m_child, reinsert_list):
                continue
