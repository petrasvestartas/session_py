...
class SpatialOctree:
    ...
    @classmethod
    def from_coords(
        cls, coords: list[float], root_spacing: float, leaf_capacity: int
    ) -> SpatialOctree:
    ...
    def _build(
        self, coords: list[float], root_spacing: float, leaf_capacity: int
    ) -> None:
    ...
    def _accept(
        self, coords: list[float], task: _Task, indices: list[int]
    ) -> list[int]:
        ...
        for i in range(task.lo, task.hi):
            ...
            for k in range(3):
                key[k] = min(
                    max(
                        math.floor((coords[idx * 3 + k] - task.min[k]) / task.spacing),
                        0,
                    ),
                    cells - 1,
                )
