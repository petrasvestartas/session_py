...
def _v_cross_product(p1: _BIVec2, p2: _BIVec2, p3: _BIVec2) -> float:
    return float(p2.x - p1.x) * float(p3.y - p2.y) - float(p2.y - p1.y) * float(
        p3.x - p2.x
    )
...
def _v_is_collinear(p1: _BIVec2, shared: _BIVec2, p2: _BIVec2) -> bool:
    return _v_products_equal(
        shared.x - p1.x, p2.y - shared.y, shared.y - p1.y, p2.x - shared.x
    )
...
def _v_get_seg_isect_pt(
    a: _BIVec2, b: _BIVec2, c: _BIVec2, d: _BIVec2
) -> tuple[bool, _BIVec2]:
    dx1 = float(b.x - a.x)
    dy1 = float(b.y - a.y)
    dx2 = float(d.x - c.x)
...
def _v_ring_size(op: _VOutPt) -> int:
...
def _v_area_tri(p1: _BIVec2, p2: _BIVec2, p3: _BIVec2) -> float:
    return (
        float(p3.y + p1.y) * float(p3.x - p1.x)
        + float(p1.y + p2.y) * float(p1.x - p2.x)
        + float(p2.y + p3.y) * float(p2.x - p3.x)
    )
...
def _v_very_small_tri(op: _VOutPt) -> bool:
    return op.next.next is op.prev and (
        _v_pts_close(op.prev.pt, op.next.pt)
        or _v_pts_close(op.pt, op.next.pt)
        or _v_pts_close(op.pt, op.prev.pt)
    )
...
def _v_valid_closed(op: _VOutPt) -> bool:
    return (
        op is not None
        and op.next is not op
        and op.next is not op.prev
        and not _v_very_small_tri(op)
    )
...
def _v_add_path_from_doubles(
    coords: list[float], n: int, polytype: int, bool_scale: float, sc: _VattiScratch
) -> tuple[_VVertex | None, int, int, int, int]:
    if n < 3:
        return None, 0, 0, 0, 0
...
def _v_is_valid_ael_order(resident: _VActive, newcomer: _VActive) -> bool:
    ...
    if not _v_is_maxima_e(resident) and resident.top.y > newcomer.top.y:
        return (
            _v_cross_product(newcomer.bot, resident.top, _v_next_vertex(resident).pt)
            <= 0
        )

    if not _v_is_maxima_e(newcomer) and newcomer.top.y > resident.top.y:
        return (
            _v_cross_product(newcomer.bot, newcomer.top, _v_next_vertex(newcomer).pt)
            >= 0
        )
    ...
    return (
        _v_cross_product(
            _v_prev_prev_vertex(resident).pt,
            newcomer.bot,
            _v_prev_prev_vertex(newcomer).pt,
        )
        > 0
...
def _v_pop_locmin(sc: _VattiScratch, y: int) -> tuple[bool, _VLocalMinima | None]:
    if (
        sc.locmin_idx >= len(sc.locmin_list)
        or sc.locmin_list[sc.locmin_idx].vertex.pt.y != y
    ):
        return False, None
        ...
        if e2.wind_cnt * e2.wind_dx < 0:
            if abs(e2.wind_cnt) > 1:
                e.wind_cnt = (
                    ...
                    else e2.wind_cnt + e.wind_dx
                )
            else:
                e.wind_cnt = e.wind_dx
        else:
            e.wind_cnt = (
                e2.wind_cnt if e2.wind_dx * e.wind_dx < 0 else e2.wind_cnt + e.wind_dx
            )
...
def _v_add_local_min_poly(
    e1: _VActive, e2: _VActive, pt: _BIVec2, sc: _VattiScratch, is_new: bool
) -> _VOutPt:
...
def _v_add_local_max_poly(
    e1: _VActive, e2: _VActive, pt: _BIVec2, sc: _VattiScratch
) -> _VOutPt | None:
    if _v_is_joined(e1):
        _v_split(e1, pt, sc)
...
def _v_check_join_left(
    e: _VActive, pt: _BIVec2, sc: _VattiScratch, check_curr_x: bool = False
) -> None:
    prev = e.prev_in_ael
    ...
    if (
        prev is None
        or not _v_is_hot(e)
        or not _v_is_hot(prev)
        or _v_is_horizontal(e)
        or _v_is_horizontal(prev)
    ):
        return
...
def _v_check_join_right(
    ...
    if (
        nxt is None
        or not _v_is_hot(e)
        or not _v_is_hot(nxt)
        or _v_is_horizontal(e)
        or _v_is_horizontal(nxt)
    ):
        return
...
def _v_intersect_edges(
    ...
    if _v_is_hot(e1) and _v_is_hot(e2):
        if (
            (old_e1_wc != 0 and old_e1_wc != 1)
            or (old_e2_wc != 0 and old_e2_wc != 1)
            or (_v_polytype(e1) != _v_polytype(e2))
        ):
...
def _v_horz_meet_maxima(
    horz: _VActive, e: _VActive, vertex_max: _VVertex, is_ltr: bool, sc: _VattiScratch
) -> None:
...
def _v_horz_stops_at(
    ...
    if vertex_max is horz.vertex_top:
        return False

    if (is_ltr and e.curr_x > horz_right) or (not is_ltr and e.curr_x < horz_left):
        return True

    if e.curr_x != horz.top.x or _v_is_horizontal(e):
        return False

    pt2 = _v_next_vertex(horz).pt

    if is_ltr:
        return _v_top_x(e, pt2.y) >= pt2.x

    return _v_top_x(e, pt2.y) <= pt2.x


def _v_horz_cross_edge(
    horz: _VActive, e: _VActive, is_ltr: bool, sc: _VattiScratch, cliptype: int
) -> _VActive | None:
    ...
    return horz.next_in_ael if is_ltr else horz.prev_in_ael
    ...
    for _ in range(max_iter):
        ...
        while e is not None:
            if e.vertex_top is vertex_max:
                _v_horz_meet_maxima(horz, e, vertex_max, is_ltr, sc)

                return

            if _v_horz_stops_at(horz, e, vertex_max, is_ltr, horz_left, horz_right):
                break

            e = _v_horz_cross_edge(horz, e, is_ltr, sc, cliptype)
...
def _v_set_horz_segment(hs: _VHorzSeg, op_p: _VOutPt, op_n: _VOutPt) -> bool:
...
def _v_update_horz_segment(hs: _VHorzSeg) -> bool:
    ...
    return _v_set_horz_segment(hs, op_p, op_n)
    ...
    for i in range(valid - 1):
        ...
        for k in range(i + 1, valid):
            ...
            if (
                ...
                or hs2.left_to_right == hs1.left_to_right
                or hs2.right_op.pt.x <= hs1.left_op.pt.x
            ):
                continue
...
def _v_add_new_isect_node(
    sc: _VattiScratch, e1: _VActive, e2: _VActive, top_y: int
) -> None:
    ...
    if ip.y > sc.bot_y or ip.y < top_y:
        ...
        if ad1 > 100 and ad2 > 100:
            ip = (
                _v_closest_pt_on_seg(ip, e1.bot, e1.top)
                if ad1 > ad2
                else _v_closest_pt_on_seg(ip, e2.bot, e2.top)
            )
        elif ad1 > 100:
...
def _v_intersect_node_key(n: _VIntersectNode) -> tuple:
    return (-n.pt.y, n.pt.x)


def _v_process_intersect_list(sc: _VattiScratch, cliptype: int) -> None:
    sc.intersect_nodes.sort(key=_v_intersect_node_key)

    for i in range(len(sc.intersect_nodes)):
        node = sc.intersect_nodes[i]

        if not (
            node.edge1.next_in_ael is node.edge2 or node.edge1.prev_in_ael is node.edge2
        ):
            for j_idx in range(i + 1, len(sc.intersect_nodes)):
                ...
                if n2.edge1.next_in_ael is n2.edge2 or n2.edge1.prev_in_ael is n2.edge2:
                    sc.intersect_nodes[i], sc.intersect_nodes[j_idx] = (
                        sc.intersect_nodes[j_idx],
                        sc.intersect_nodes[i],
                    )
                    node = sc.intersect_nodes[i]
                    break
...
def _v_insert_local_minima_into_ael(
    ...
    while True:
        ...
        while rb.next_in_ael is not None and _v_is_valid_ael_order(rb.next_in_ael, rb):
            _v_intersect_edges(
                rb, rb.next_in_ael, _BIVec2(rb.bot.x, rb.bot.y), sc, cliptype
            )
...
# Sweep
# ═══════════════════════════════════════════════════════════════════════════
def _v_locmin_key(lm: _VLocalMinima) -> tuple:
    return (-lm.vertex.pt.y, lm.vertex.pt.x)


def _v_execute_internal(sc: _VattiScratch, cliptype: int) -> bool:
    sc.locmin_list.sort(key=_v_locmin_key)
...
def _v_select(
    a: Polyline, b: Polyline, a_in_b: bool, b_in_a: bool, clip_type: int
) -> list[Polyline]:
...
def _v_select_count(
    a_count: int, b_count: int, a_in_b: bool, b_in_a: bool, clip_type: int
) -> int:
    if clip_type == 0:
        if a_in_b:
            return a_count
...
def _v_any_cross(va: list[_BIVec2], vb: list[_BIVec2]) -> bool:
    ...
    for i in range(na):
        ...
        for j in range(nb):
            ...
            if (
                max(b1.x, b2.x) < axmin
                or min(b1.x, b2.x) > axmax
                or max(b1.y, b2.y) < aymin
                or min(b1.y, b2.y) > aymax
            ):
                continue
...
def _v_add_small_paths(
    ...
    for i in range(na):
        va.append(_v_cvt_to_i64(a.coords, i * 3, bool_scale))

    for i in range(nb):
        vb.append(_v_cvt_to_i64(b.coords, i * 3, bool_scale))
...
def _v_add_large_paths(
    ...
    if a_max_x < b_min_x or b_max_x < a_min_x or a_max_y < b_min_y or b_max_y < a_min_y:
        return _v_select(
            a,
            b,
            _pip_vertex(va_head.pt, vb_head),
            _pip_vertex(vb_head.pt, va_head),
            clip_type,
        )
...
class BooleanPolyline:
    ...
    def compute(a: Polyline, b: Polyline, clip_type: int) -> list[Polyline]:
        ...
        if na * nb <= 400:
            early = _v_add_small_paths(a, b, na, nb, bool_scale, clip_type, sc)
        else:
            early = _v_add_large_paths(a, b, na, nb, bool_scale, clip_type, sc)
