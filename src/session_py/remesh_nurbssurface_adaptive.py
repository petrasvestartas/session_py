...
class _Node:
    ...
    def __init__(
        ...
        """Construct a leaf over [u0, u1] x [v0, v1] from its five corners."""

        self.u0 = u0  # Lower u bound.
        self.v0 = v0  # Lower v bound.
        self.u1 = u1  # Upper u bound.
        self.v1 = v1  # Upper v bound.
        self.c = c  # Corners SW SE NE NW and the centre.
        self.depth = depth  # Subdivision depth from the root.
        self.leaf = True  # True until the cell is split.
...
class _Quadtree:
    ...
    def __init__(
        ...
        self.s = s  # Surface sampled.
        self.usp = s.get_span_vector(0)  # Span vector in u.
        self.vsp = s.get_span_vector(1)  # Span vector in v.
        self.closed = [
            s.is_closed(0),
            s.is_closed(1),
        ...
        self.min_edge = min_edge  # Shortest cell edge still split, 0 for no limit.
        self.nodes: list[_Node] = []  # Cell pool, root cells first.
        self.corners: dict[tuple[int, int], _Corner] = {}  # Leaf corners by key.
        self.rows: dict[int, list[int]] = {}  # U keys on each row.
        self.cols: dict[int, list[int]] = {}  # V keys on each column.
        self.mesh = Mesh()  # Mesh emitted.
        self.keys: dict[tuple[int, int], int] = {}  # Mesh vertex per key.
        self.south: int | None = None  # Pole vertex on the v0 side.
        self.north: int | None = None  # Pole vertex on the v1 side.
...
class RemeshNurbsSurfaceAdaptive:
    ...
    def __init__(self, surface: NurbsSurface):
        ...
        self._surface = surface  # Surface to mesh.
        self._max_angle = 20.0  # Largest normal turn across a cell in degrees.
        self._max_edge_length = 0.0  # Longest cell edge, 0 for no limit.
