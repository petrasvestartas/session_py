...
class VertexData:
    ...
    def color(self) -> list[float]:
        ...
        return [self.attributes.get("r", 0.5), self.attributes.get("g", 0.5), self.attributes.get("b", 0.5)]
...
def _planar_cdt(pts: list[Point]) -> list:
    ...
    bpts = []
    for p in pts:
        bpts.append(Point(p[0] * ux + p[1] * uy + p[2] * uz, p[0] * vx + p[1] * vy + p[2] * vz, 0.0))
...
class LoftResult:
    def __init__(self, panels: list[LoftPanel], adjacency: list[LoftAdjPair], top_mesh: "Mesh", bot_mesh: "Mesh"):
        self.panels = panels
...
def _loft_frame(bottom: Polyline, top: Polyline) -> _LoftFrame:
    ...
    c0 = bottom.center()
    c1 = top.center()
    bottom_to_top = Vector(c1[0] - c0[0], c1[1] - c0[1], c1[2] - c0[2])
...
def _loft_drop_degenerate(tris: list[list[int]], mesh: "Mesh", frame: _LoftFrame) -> list[list[int]]:
...
def _loft_wall_start(frame: _LoftFrame, bpts: list[Point], tpts: list[Point]) -> tuple[int, int]:
...
def _loft_same_point(a: Point, b: Point) -> bool:
    return abs(a[0] - b[0]) < 1e-10 and abs(a[1] - b[1]) < 1e-10 and abs(a[2] - b[2]) < 1e-10
...
def _lp_merge_collinear(pts: list[Point], vkeys: list[int]) -> tuple[list[Point], list[int]]:
...
def _lp_merge_close(pts: list[Point], vkeys: list[int]) -> tuple[list[Point], list[int]]:
...
def _lp_match_faces(top_mesh: "Mesh", bot_mesh: "Mesh") -> list[tuple[int, int]]:
    ...
    for ti in range(len(tfks)):
        for bi in range(len(bfks)):
            dists.append((_lp_face_centroid(top_mesh, tfks[ti]).distance(_lp_face_centroid(bot_mesh, bfks[bi])), ti, bi))
    dists.sort()
...
def _lp_orient_rings(top_pts: list[Point], top_vkeys: list[int], bot_pts: list[Point], bot_vkeys: list[int]) -> None:
    ...
    axis = Vector(tc[0] - bc[0], tc[1] - bc[1], tc[2] - bc[2])
...
def _lp_midpoints(pts: list[Point]) -> list[Point]:
    ...
    mids = []
    for i in range(n):
        mids.append(Point((pts[i][0] + pts[(i + 1) % n][0]) * 0.5, (pts[i][1] + pts[(i + 1) % n][1]) * 0.5, (pts[i][2] + pts[(i + 1) % n][2]) * 0.5))
    return mids
...
def _lp_add_quad(panel: LoftPanel, b0: int, b1: int, t0: int, t1: int, edge_gap: float) -> int | None:
...
def _lp_build_panel(
    ...
    _lp_add_walls(panel, top_pts, top_vkeys, bot_pts, bot_vkeys, edge_gap, edge_match_threshold, skip_triangles)
    if add_caps:
    ...
    for w in panel.wall_faces:
        ...
        panel.face_roles[w.face_key] = LoftFaceRole.QuadWall if w.is_quad else LoftFaceRole.TriWall
    ...
    for pi in range(len(panels)):
        for wi in range(len(panels[pi].wall_faces)):
            ...
            if key in edge_to_wall and edge_to_wall[key][0] > pi:
                adjacency.append(LoftAdjPair(pi, wi, edge_to_wall[key][0], edge_to_wall[key][1]))
    return adjacency
...
def _fold_chamfer_mask(pts: list[Point], max_angle_deg: float) -> list[bool]:
    ...
    for i in range(n):
        prev = (i + n - 1) % n
        nxt = (i + 1) % n
        dp = Vector(pts[prev][0] - pts[i][0], pts[prev][1] - pts[i][1], pts[prev][2] - pts[i][2])
        dn = Vector(pts[nxt][0] - pts[i][0], pts[nxt][1] - pts[i][1], pts[nxt][2] - pts[i][2])
        lp = dp.magnitude()
        ln = dn.magnitude()
...
def _fold_chamfer(pts: list[Point], s: float, mask: list[bool]) -> list[Point]:
    ...
    for i in range(n):
        j = (i + 1) % n
        d = Vector(pts[j][0] - pts[i][0], pts[j][1] - pts[i][1], pts[j][2] - pts[i][2])
    ...
    for i in range(n):
        ...
        result.append(Point(pts[i][0] + dp[0] * sp, pts[i][1] + dp[1] * sp, pts[i][2] + dp[2] * sp))
        result.append(Point(pts[i][0] + dn[0] * sn, pts[i][1] + dn[1] * sn, pts[i][2] + dn[2] * sn))
    return result
...
def _miter_planes(shell: "Mesh", efm: dict, fverts: list[int], pts: list[Point], fn: Vector) -> list[Plane]:
    ...
    for i in range(n):
        ...
        if adj_fk is not None:
            ...
            total = Vector(fn[0] + adj[0], fn[1] + adj[1], fn[2] + adj[2])
            if total.magnitude() > 0.1 and total.normalize_self():
                avg_n = total
        edge_dir = Vector(pts[j][0] - pts[i][0], pts[j][1] - pts[i][1], pts[j][2] - pts[i][2])
        if not edge_dir.normalize_self():
            return []
        mn = avg_n.cross(edge_dir)
        if not mn.normalize_self():
            return []
        mid = Point((pts[i][0] + pts[j][0]) / 2, (pts[i][1] + pts[j][1]) / 2, (pts[i][2] + pts[j][2]) / 2)
...
class Mesh:
    ...
    def __init__(self):
        ...
        self.default_vertex_attributes: dict[str, float] = {"x": 0.0, "y": 0.0, "z": 0.0}
    ...
    def __deepcopy__(self, memo):
        ...
        m.triangulation = {k: [list(t) for t in v] for k, v in self.triangulation.items()}
        memo[id(self)] = m
        return m
    ...
    @staticmethod
    def from_vertices_and_faces(vertices: list[Point], faces: list[list[int]]) -> "Mesh":
    ...
    @staticmethod
    def _polylines_vertex_key(mesh: "Mesh", p: Point, precision: float | None, map_eps: dict, map_exact: dict) -> int:
    ...
    @staticmethod
    def from_polylines(polygons: list[list[Point]], precision: float | None = None) -> "Mesh":
        ...
        for poly in polygons:
            ...
            vkeys = []
            for p in poly:
                vkeys.append(Mesh._polylines_vertex_key(mesh, p, precision, map_eps, map_exact))
    ...
    @staticmethod
    def from_lines(lines: list[Line], delete_boundary_face: bool = False, precision: float | None = None) -> "Mesh":
    ...
    @staticmethod
    def from_polygon_with_holes(polylines: list[list[Point]], sort_by_bbox: bool = False) -> "Mesh":
    ...
    @staticmethod
    def loft(polylines0: list[Polyline], polylines1: list[Polyline], cap: bool = True, fix_collinear: bool = True) -> "Mesh":
        ...
        for oi in range(len(order)):
            ...
            if (area < 0) if oi == 0 else (area > 0):
                bot.reverse()
                top.reverse()
            polys.append(_LoftPoly(_LoftRing(len(all_bot), len(bot)), _LoftRing(len(all_top), len(top))))
            all_bot.extend(bot)
            all_top.extend(top)
        mesh = Mesh()
    ...
    def from_polygon_with_holes_many(inputs: list, sort_by_bbox: bool = False, parallel: bool = True) -> list["Mesh"]:
        ...
        if parallel and len(inputs) > 1:
            ...
            with ThreadPoolExecutor() as pool:
                return list(pool.map(lambda x: Mesh.from_polygon_with_holes(x, sort_by_bbox), inputs))
        results = []
        for x in inputs:
    ...
    @staticmethod
    def loft_many(pairs: list, cap: bool = True, parallel: bool = True, fix_collinear: bool = True) -> list["Mesh"]:
        """Batch loft, parallel when asked"""
        if parallel and len(pairs) > 1:
            from concurrent.futures import ThreadPoolExecutor
            with ThreadPoolExecutor() as pool:
                return list(pool.map(lambda p: Mesh.loft(p[0], p[1], cap, fix_collinear), pairs))
        results = []
        for p in pairs:
    ...
    def loft_panels(
        ...
        for tfk, bfk in face_match:
            panels.append(_lp_build_panel(top_mesh, bot_mesh, tfk, bfk, edge_gap, edge_match_threshold, add_caps, skip_triangles))
    ...
    def create_dodecahedron(edge: float = 2.0) -> "Mesh":
        ...
        faces = []
        for f in idx:
            faces.append([verts[f[0]], verts[f[1]], verts[f[2]], verts[f[3]], verts[f[4]]])
    ...
    @staticmethod
    def from_polyline_pairs_vnf(pairs: list[Polyline], scale: float = 1.0) -> tuple[list[float], list[float], list[int]]:
    ...
    def reflex_fold(cross_section: Polyline, profile: Polyline) -> "Mesh":
        ...
        for i in range(n_cs):
            ...
            if i > 0 and i < n_cs - 1:
                ...
                v1 = Vector(cp[0] - ci[0], cp[1] - ci[1], cp[2] - ci[2]).normalized()
                v2 = Vector(cn[0] - ci[0], cn[1] - ci[1], cn[2] - ci[2]).normalized()
        ...
        for i in range(1, n_cs):
            ...
            n1 = Vector(po[0] - pp[0], po[1] - pp[1], po[2] - pp[2])
            n2 = planes[i].z_axis()
            row_start = len(all_pts)
            for j in range(n_p):
                pvrt = all_pts[row_start - n_p + j]
                diff = Vector(po[0] - pvrt[0], po[1] - pvrt[1], po[2] - pvrt[2])
                denom = n2.dot(n1)
                t = n2.dot(diff) / denom if abs(denom) > 1e-12 else 0.0
                all_pts.append(Point(pvrt[0] + n1[0] * t, pvrt[1] + n1[1] * t, pvrt[2] + n1[2] * t))
    ...
    def miter_contours(
        ...
        for fk in shell.faces():
            ...
            if len(corner_lines) != n:
                continue
            bot_origin = Point(cen[0] + fn[0] * 2.0 * thickness, cen[1] + fn[1] * 2.0 * thickness, cen[2] + fn[2] * 2.0 * thickness)
            top_contour = _miter_contour(corner_lines, Plane.from_point_normal(cen, fn))
            bot_contour = _miter_contour(corner_lines, Plane.from_point_normal(bot_origin, fn))
        ...
        for seed in self.faces():
            ...
            while queue:
                ...
                for i in range(n):
                    ...
                    for adj_key, adj_u, adj_v in edge_faces.get((min(u_orig, v_orig), max(u_orig, v_orig)), []):
    ...
    def halfedge_before(self, edge: tuple[int, int]) -> tuple[int, int] | None:
        ...
        for w in sorted(nbrs.keys()):
            if w == v:
                continue
            if w in self.halfedge and u in self.halfedge[w] and self.halfedge[w][u] is None:
                return (w, u)
        return None
    ...
    def set_edge_attribute(self, edge: tuple[int, int], name: str, value: float) -> None:
        u, v = edge
        key = (v, u) if (v, u) in self.edgedata else (u, v)
        self.edgedata.setdefault(key, {})[name] = value

    def vertices_attribute(self, name: str, keys: list[int] | None = None) -> list[float | None]:
    ...
    def set_vertices_attribute(self, name: str, value: float, keys: list[int] | None = None) -> None:
        if keys is None:
            keys = self.vertices()
        for k in keys:
            self.set_vertex_attribute(k, name, value)

    def faces_attribute(self, name: str, keys: list[int] | None = None) -> list[float | None]:
        if keys is None:
            keys = self.faces()
        out = []
        for k in keys:
    ...
    def set_faces_attribute(self, name: str, value: float, keys: list[int] | None = None) -> None:
        if keys is None:
            keys = self.faces()
        for k in keys:
            self.set_face_attribute(k, name, value)

    def edges_attribute(self, name: str, keys: list[tuple[int, int]] | None = None) -> list[float | None]:
        if keys is None:
            keys = self.edges()
        out = []
        for e in keys:
    ...
    def set_edges_attribute(self, name: str, value: float, keys: list[tuple[int, int]] | None = None) -> None:
        if keys is None:
            keys = self.edges()
        for e in keys:
    ...
    def vertices_where_predicate(self, pred: Callable[[int, dict[str, float]], bool]) -> list[int]:
        out = []
        for k in self.vertices():
    ...
    def faces_where_predicate(self, pred: Callable[[int, dict[str, float]], bool]) -> list[int]:
        out = []
        for k in self.faces():
    ...
    def edges_where_predicate(self, pred: Callable[[tuple[int, int], dict[str, float]], bool]) -> list[tuple[int, int]]:
        out = []
        for e in self.edges():
    ...
    def _dihedral_arm(centroid: Point, mid: Point, edge: Vector) -> Vector | None:
        ...
        d = Vector(centroid[0] - mid[0], centroid[1] - mid[1], centroid[2] - mid[2])
        dot = d.dot(edge)
        d = Vector(d[0] - dot * edge[0], d[1] - dot * edge[1], d[2] - dot * edge[2])
        length = d.magnitude()
        if length < 1e-10:
            return None
        return Vector(d[0] / length, d[1] / length, d[2] / length)
    ...
    def dihedral_angles(self, scale: float = 0.3, with_arcs: bool = True, with_points: bool = True) -> tuple[dict[tuple[int, int], float], list[Polyline], list[Point]]:
        ...
        for u, v in self.edges():
            ...
            mid = Point((ep0[0] + ep1[0]) * 0.5, (ep0[1] + ep1[1]) * 0.5, (ep0[2] + ep1[2]) * 0.5)
            if scale == 0.0:
                if not with_points:
                    continue
                ...
                continue
            ef = self.edge_faces(u, v)
            edge = Vector(ep1[0] - ep0[0], ep1[1] - ep0[1], ep1[2] - ep0[2])
            ...
            if with_arcs:
                ...
                arcs.append(arc)
            if with_points:
                pt = Point(arc_pts[arc_n // 2][0], arc_pts[arc_n // 2][1], arc_pts[arc_n // 2][2], str(da))
                pt.pointcolor = label_color
                points.append(pt)
    ...
    def vertex_normal_weighted(self, vertex_key: int, weighting: NormalWeighting) -> Vector | None:
        ...
        if length > Tolerance.ZERO_TOLERANCE:
            return Vector(normal_acc[0] / length, normal_acc[1] / length, normal_acc[2] / length)
        return None
    ...
    def _corner_angle(pts: list[Point], i: int) -> float:
        ...
        prev = (i + n - 1) % n
        nxt = (i + 1) % n
        a = Vector(pts[prev][0] - pts[i][0], pts[prev][1] - pts[i][1], pts[prev][2] - pts[i][2])
        b = Vector(pts[nxt][0] - pts[i][0], pts[nxt][1] - pts[i][1], pts[nxt][2] - pts[i][2])
        a_len = a.magnitude()
        b_len = b.magnitude()
    ...
    def _triangle_tasks(self, faces: list[list[int]]) -> list[tuple[int, int, int, int, int]]:
    ...
    def build_triangle_bvh(self, force: bool = False) -> None:
        ...
        for i0, i1, i2, face_idx, sub_idx in tasks:
            self._triangle_aabbs_cache.append(Mesh._triangle_aabb(vertices[i0], vertices[i1], vertices[i2]))
        ...
        self._triangle_bvh.build_from_aabbs(self._triangle_aabbs_cache, Mesh._triangle_world_size(self._triangle_aabbs_cache))
        self._triangle_bvh_built = True

    def triangle_bvh_ray_cast(self, origin: Point, direction: Vector, candidate_ids: list[int], find_all: bool = False) -> bool:
    ...
    def get_triangle_by_id(self, tri_id: int) -> tuple[bool, int, int, Point | None, Point | None, Point | None]:
        """Face index, sub-triangle index and corners of a cached triangle id as (found, face_idx, sub_idx, v0, v1, v2)"""
        if tri_id < 0:
        ...
        if tri[0] >= len(self._vertices_cache) or tri[1] >= len(self._vertices_cache) or tri[2] >= len(self._vertices_cache):
            return (False, 0, 0, None, None, None)
        return (True, face_idx, sub_idx, self._vertices_cache[tri[0]], self._vertices_cache[tri[1]], self._vertices_cache[tri[2]])
    ...
    def __jsondump__(self) -> dict:
        ...
        data["guid"] = self.guid
        he = self.compute_halfedges() if not self.halfedge and self.face else self.halfedge
        halfedge_json = {}
        ...
        vertex_json = {}
        for key, vdata in self.vertex.items():
            vertex_json[str(key)] = {"attributes": dict(vdata.attributes), "x": vdata.x, "y": vdata.y, "z": vdata.z}
        data["vertex"] = vertex_json
        data["widths"] = list(self._widths)
        return data

    @classmethod
    def __jsonload__(cls, data: dict, guid: str | None = None, name: str | None = None) -> "Mesh":
        mesh = cls()
        if "guid" in data:
            mesh.guid = data["guid"]
        if "name" in data:
    ...
    def from_proto(cls, proto: "mesh_pb2.Mesh") -> "Mesh":
        ...
        mesh.color_mode = list(ColorMode)[proto.color_mode] if 0 <= proto.color_mode < 4 else ColorMode.OBJECTCOLOR
        if mesh.vertex:
