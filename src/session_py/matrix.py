...
class Matrix:
    ...
    def from_list(rows: int, cols: int, data: list[float]) -> "Matrix":
        """Construct from row-major values using the legacy list API.
        ...
        -------
        Matrix
            Matrix containing floating-point copies of ``data``.
        ...
        Examples
        --------
        >>> Matrix.from_list(1, 2, [3, 4]).data
        [3.0, 4.0]
        """
        return Matrix.from_vec(rows, cols, [float(value) for value in data])
    ...
    def __mul__(self, other: Union["Matrix", float]) -> "Matrix":
        """Multiply by a compatible matrix or scale by a scalar."""
