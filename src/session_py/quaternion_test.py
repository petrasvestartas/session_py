...
def test_quaternion_constructor():
    ...
    MINI_CHECK(len(q0.guid) > 0)
    MINI_CHECK(TOLERANCE.is_close(q0.scalar, 1.0))
    MINI_CHECK(
        TOLERANCE.is_close(q0.vector[0], 0.0)
        and TOLERANCE.is_close(q0.vector[1], 0.0)
        and TOLERANCE.is_close(q0.vector[2], 0.0)
    )
    ...
    MINI_CHECK(
        qrepr == "Quaternion(my_quaternion, 5.000000, 0.000000, 1.000000, 0.000000)"
    )
    MINI_CHECK(qcopy == q and qcopy.guid != q.guid)
    MINI_CHECK(qother != q)
    MINI_CHECK(
        TOLERANCE.is_close(qmul.scalar, 0.0) and TOLERANCE.is_close(qmul.vector[2], 1.0)
    )
    MINI_CHECK(TOLERANCE.is_close(qscaled.scalar, 2.0))
    MINI_CHECK(
        TOLERANCE.is_close(qsum.scalar, 1.0) and TOLERANCE.is_close(qsum.vector[2], 1.0)
    )
    MINI_CHECK(
        TOLERANCE.is_close(qdiff.scalar, 0.0)
        and TOLERANCE.is_close(qdiff.vector[2], 0.0)
    )
...
def test_quaternion_from_rotation():
    ...
    plane_a = Plane.xy_plane()
    plane_b = Plane.from_frame(
        Point(0.0, 0.0, 0.0),
        Vector(0.0, 1.0, 0.0),
        Vector(-1.0, 0.0, 0.0),
        Vector(0.0, 0.0, 1.0),
